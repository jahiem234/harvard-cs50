...
int main(int argc, char *argv[])
{
    ...
    while (fread(block, sizeof(block), 1, inptr) == 1)
    {
        ...
        else
        {
            ...
            if (foundjpg == true)
            {
                // This is not a header and there is already a jpg, so write this block to current
                // jpg.
                fwrite(block, sizeof(block), 1, JPG);
            }
            // We only land here if we're in the beginning and still haven't found a header.
            // We check again. If there happen to be zero headers in the whole file, loop exits and
            // file closes.
        }
    }
